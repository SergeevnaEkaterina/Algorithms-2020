...
public class JavaTasks {
     ...
     * 121.3
     */
    //Трудоёмкость = O(N), так как сложность прохождения итератором = O(N), сложность записи в map O(logN) = const, так как диапазон = константе
    //Ресурсоёмкость = О(1), так как ограниченный(константный) диапазон температур
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(inputName))) {
            while ((line = reader.readLine()) != null) {
                ...
                if (!temperature.containsKey(stringToDouble)) {
                    ...
                    temperature.put(stringToDouble, temperature.get(stringToDouble) + 1); //увеличиваем количество повторов
            }
     ...
     * 2
     */
    //Трудоёмкость = O(N*logN), Сложность операций для TreeMap = O(logN), выполняем для каждого числа в файле = O(N)
